...
const FloorMap = forwardRef(({ onRoomSelect, isPanelOpen, selectedRoomId }, ref) => {
    ...
    useEffect(() => {
        if (selectedRoomId && transformComponentRef.current) {
            ...
            // Delay zoom if the panel is just opening (to wait for layout transition)
            const delay = (!wasPanelOpen.current && isPanelOpen) ? 600 : 0;

            setTimeout(() => {
                // We target the ID we added to the <rect> below
    ...
    const getRoomStyle = (room, isSelected) => {
        ...
        const selectedFills = { blue: "fill-[#abc9fe]", red: "fill-[#fba1a4]", green: "fill-[#98e7b9]", default: "fill-gray-500/60" };
        const idleFills = { blue: "fill-[#d5e5ff] hover:fill-[#bbd6ff]", red: "fill-[#fed5d7] hover:fill-[#ffc2c5]", green: "fill-[#ccf4dc] hover:fill-[#b2eecb]", default: "fill-gray-500/20 hover:fill-gray-500/40" };
        const strokeStyles = { blue: "stroke-blue-600", red: "stroke-red-600", green: "stroke-green-600", default: "stroke-gray-600" };

        if (isSelected) classes += (selectedFills[colorType] || selectedFills.default) + " ";
        else classes += (idleFills[colorType] || idleFills.default) + " ";
        ...
        <div className="w-full h-full bg-white flex items-center justify-center relative overflow-hidden">
            ...
            >
                <TransformComponent wrapperStyle={{ width: "100%", height: "100%" }}>
                    <div style={{ width: `${MAP_WIDTH}px`, height: `${MAP_HEIGHT}px` }} className="relative bg-white shadow-2xl rounded-sm">
                        ...
                        <svg viewBox={`0 0 ${MAP_WIDTH} ${MAP_HEIGHT}`} className="absolute top-0 left-0 w-full h-full">
                            {rooms.map((room) => {
                                ...
                                return (
                                    <g key={room.id}>
                                        {/* FIXED: Added id={room.id} back so zoomToElement can find it */}
                                        <rect
                                            id={room.id}
                                        ...
                                        />
                                        <text x={centerX} y={centerY - ((lines.length - 1) * lineHeight) / 2} textAnchor="middle" dominantBaseline="middle" className="pointer-events-none select-none fill-slate-800 font-bold opacity-75" style={{ fontSize: "20px", textShadow: "0px 0px 4px rgba(255,255,255,0.8)" }}>
                                            {lines.map((line, index) => <tspan key={index} x={centerX} dy={index === 0 ? 0 : lineHeight}>{line}</tspan>)}
                                        </text>
                                    </g>
                                );
                            })}
